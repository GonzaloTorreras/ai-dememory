...
micromanagement. ADR 0247 introduced tag-driven releases and ADR 0251 introduced
a tuple-bound bot approval after an independent read-only review. Both are
valuable technical controls, but their original wording also treated green
automation as sufficient authority to merge and publish.
...
Keep the tag-driven release and tuple-bound auto-approval mechanisms, with these
...
- Merge, release-tag creation or dispatch, package publication, repository
  visibility changes, and secret or trusted-publisher changes require explicit
...
- Release authorization is a separate manual action bound to the exact tag and
  current-main SHA; approval to merge does not implicitly approve that dispatch.
...
artifact evidence, and the exact manual tag-dispatch tuple.
...
- ADR 0247 defines immutable, tag-driven package releases.
- ADR 0251 defines tuple-bound technical auto-approval.
- ADR 0258 defines tuple-bound manual release-tag authorization.
