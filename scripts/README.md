...
- `ci_guard.py`: binds the exact checkout/setup/command inventory to the single
  static, non-skippable `verify` job, rejects forgeable workflow equivalents,
  and validates the isolated manual-only GitHub Pages workflow boundary.
